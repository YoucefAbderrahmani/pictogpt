...
/** Line 1 + newline + compact line; e.g. `1)The stem …\n1A-2BC-3S` (multi-select = multiple letters after q). */
export function hasQcmStemHeaderFormat(s) {
  ...
  if (!/^\d{1,6}\)/.test(line1)) return false;
...
export function normalizeAnswerChoices(raw) {
  ...
  if (lettersOnly.includes('S')) return sortUniqueOptionLetters(noS);
  return sortUniqueOptionLetters(lettersOnly);
}

/** Compact line: `1A-2BC-12S` — each segment is `q` + `S` (skip) or `q` + one or more A–Z letters (no S inside). */
export function isValidQcmCompactLine(compact) {
  ...
  for (const p of parts) {
    ...
    if (tail === 'S') continue;
    if (!/^[A-Z]+$/.test(tail) || tail.includes('S')) return false;
  ...
  try {
    ...
    const normalized = answers
      .map((entry) => ({
        ...
        a: normalizeAnswerChoices(entry?.a ?? entry?.A ?? ''),
      }))
      .filter(
        (x) =>
          Number.isFinite(x.q) &&
          x.q >= 1 &&
          x.q <= 999999 &&
          (x.a === 'S' || (/^[A-Z]{1,24}$/.test(x.a) && !x.a.includes('S')))
      );
