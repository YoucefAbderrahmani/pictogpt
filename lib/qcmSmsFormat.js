/** Three spaces before the quoted chosen-option text (per SMS layout). */
...
/** Compact-only prefix (strip optional `   "…"` tail). */
...
export function hasQuotedAnswerTail(s) {
  return /\s{3,}"[^"]*"\s*$/i.test(String(s ?? '').trim());
}
...
/**
 * When `smsBody` has no trailing `   "…"`, rebuild from JSON inside `modelText` (app fallback).
 */
