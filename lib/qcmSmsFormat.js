...
/** Line 1 + newline + compact line; e.g. `1)__1) The stem...\n1A-2B-3S`. */
export function hasQcmStemHeaderFormat(s) {
  ...
  if (!/^\d{1,6}[)]__\d{1,6}[)]\s+.{1,10}\.\.\.$/.test(line1)) return false;
...
export function toQcmSmsFormat(rawText) {
  ...
  if (!best) return '';
  const batches = formatBatches(best.deduped, best.parsed);
  return batches[0] || '';
}
...
export function toQcmSmsBatches(rawText) {
  const best = bestQcmCandidate(rawText);
  if (!best) return [];
  return formatBatches(best.deduped, best.parsed);
}
