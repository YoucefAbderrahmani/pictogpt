...
- **Skip leading non-question material:** If the page/image begins with text that is **not** part of an exam question (instructions, exam title, course name, duration, generic header, unrelated paragraphs, answer keys without a matching stem on prior pages, etc.), **ignore it** and start from the **first** block that is clearly a **question** (stem + its choices, or the exam’s numbered QCM pattern). Do **not** add **answers** rows for skipped preamble content.
...
**Use the printed question numbers on the page:** Whenever a **question** shows an explicit numeric index on the sheet, that integer is **q**. Match common patterns: **1.** / **1)** / **1 -** before the stem, **Q1** / **Q 1** / **Question 1** / **Quest. 1**, **§1**, etc. Strip decoration; use the integer only. **Do not renumber** what is printed (e.g. **37**, **38**, **39** on the page ⇒ **q** = **37**, **38**, **39**).

**No question numbers printed anywhere:** If **no** question on the whole set of images has any such printed **question** index, assign **q** yourself: **1**, **2**, **3**, … in **reading order** across all images (page order first, then top-to-bottom, left-to-right within each image).
